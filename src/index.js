...
function emptyCart(inside) {
	inside.innerHTML = `
	<p class="m-auto text-dark-grayish-blue">
		Your cart is empty.
	</p>`;
}
function loadCart(inside, items) {
	inside.innerHTML = `
	<section class="max-w-full max-h-full flex">
		<div class="flex flex-row gap-x-1 relative max-w-full max-h-full justify-between">
			<img
				src="./images/image-product-1-thumbnail.jpg"
				alt=""
				class="rounded-xl object-contain max-w-full h-auto w-1/5"
			/>

			<header class="py-1/2">
				<h5
					class="text-sm font-normal text-dark-grayish-blue"
				>
					Fall Limited Edition
					Sneakers
				</h5>
				<p
					class="text-dark-grayish-blue font-normal mt-[2px]"
				>
					$125.00 x ${items}
					<span
				...
				<svg
					...
					viewBox="0 0 14 16"
					class="absolute top-4 fill-[#C3CAD9] right-0 cursor-pointer hover:fill-dark-grayish-blue transition"
					id="delete"
				>
					<defs>
						<path
...
document.addEventListener("click", function (e) {
	...
	if (e.target == cart || e.target == cartPath) {
		...
		return popUp.classList.toggle("invisible");
	}
